...
bool Nivel::collision(SDL_FRect player, SDL_FRect enemy) {
    ...
    bool colisionPlayerY = (player.y >= enemy.y &&  player.y <= enemy.h) || (player.h >= enemy.h &&  player.h <= enemy.y);

    bool colisionEnemyX = (enemy.x >= player.x &&  enemy.x <= player.w) || (enemy.w >= player.x &&  enemy.w <= player.w);
    bool colisionEnemyY = (enemy.y >= player.y &&  enemy.y <= player.h) || (enemy.h >= player.h &&  enemy.h <= player.y);
